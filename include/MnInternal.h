...
MnValue ut_getglobal( MnState* s, MnValue& n )
{
	if ( MnIsString(n) )
	{
		return ut_getfield( s, s->gtable, n );
	}
	return MnValue();
}
		...
		case op_getglobal :
			{
				...
				if (op==op_setglobal)
				{
					ut_setfield( s, gtable, ut_getconst(s->func,idx), ut_getstack(s,-1) );
					mn_pop(s,1);
				}
				else
				{
					ut_pushvalue( s, ut_getfield( s, gtable, ut_getconst(s->func,idx) ) );
				}
			}
			break;
